'''3.1 script for MEP analysis (special cases)
loads epoched data from script 3 'stroke_data_epoch'
and extracts the intensity value to perform a cluster analysis. 
!! -> this script handels the special cases previously defined by visual inspection <- !!
-> by removing the outlier value

Followed by script 4. 'stroke_data_visual(special_case)'
...
timepoint = 'post5' #os.listdir(direct)
...
def load_intensities (data):
    ...
    for epoch in data.keys():
        ...
        if trial['intensity'] != 0:
            #if trial['intensity'] in [77, 56]:  #define problematic intensities visually 
                #trial['intensity'] += 1         #and shift them to higher value/cluster
            intensities.append(trial['intensity'])

    #max_epoch = intensities.indices(min(intensities)) + 1 #find epoch with largest (outlier) value
    #del data[max_epoch] # remove the corresponding epoch from data
    #intensities.remove(max(intensities)) # remove outlier value from list
    #for index in indices: #delete epochs with intensity 0
        #del data[index]
    l = len(intensities)  
...
subjects = ['007.json']#,'014.json','015.json'] #'006.json','008.json',
