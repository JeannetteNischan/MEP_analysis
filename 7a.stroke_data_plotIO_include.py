...
followed by 7.1 stroke_data_boxplots
then 8.stroke_data_stats
...
direct_IO = '/home/jeanettenischan/Data/data_INTENS_TMS/ProcessedData/IO_include/group/'
...
def group_plots(group_avg, MEP_occ, timepoints):
    ...
    for intensity in group_avg.keys():
        ...
        for time in group_avg[intensity].keys():
            ...
            groupAVG[time][intensity].append(stat.stdev(group_avg[intensity][time]))
            for i in range(0,len(group_avg[intensity][time])):
                x.append(time)
        ...
        for time in timepoints:
            ...
            groupAVG[time][intensity].append(stat.stdev(occ_time))
    ...
    for time in timepoints:
        ...
        for intensity in group_avg.keys():
            for i in range(0,len(group_avg[intensity][time])):
                ...
                y_log.append(log(group_avg[intensity][time][i] + 1))    
                x.append(intensity)
            y_avg.append(sum(group_avg[intensity][time])/len(group_avg[intensity][time]))


        for intensity in group_avg.keys():
            y_std_log = [log(val+1) for val in group_avg[intensity][time]]
            y_log_avg.append(sum(y_std_log)/len(y_std_log))
            groupLOG[time][intensity].append(sum(y_std_log)/len(y_std_log))
            std_dev.append(stat.stdev(group_avg[intensity][time]))
            std_dev_log.append(stat.stdev(y_std_log))
            groupLOG[time][intensity].append(stat.stdev(y_std_log))
        ...
        plt.errorbar(x_avg,y_avg, yerr = std_dev)
        ...
        plt.errorbar(x_avg,y_log_avg, yerr = std_dev_log)
...
with open('/home/jeanettenischan/Data/data_INTENS_TMS/ProcessedData/IO_include/groupAVG.json','w') as outfile:
    json.dump(groupAVG,outfile)

with open('/home/jeanettenischan/Data/data_INTENS_TMS/ProcessedData/IO_include/groupLOG.json','w') as outfile:
    json.dump(groupLOG,outfile)    

with open('/home/jeanettenischan/Data/data_INTENS_TMS/ProcessedData/IO_include/group_avgs.json','w') as outfile:
    json.dump(group_avg,outfile)

with open('/home/jeanettenischan/Data/data_INTENS_TMS/ProcessedData/IO_include/group_MEP_occ.json','w') as outfile:
