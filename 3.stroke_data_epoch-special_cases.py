...
Followed by script 3.1 'stroke_data_cluster' 4. 'stroke_data_visual'
...
#%% predefine paths and variables (specifications)
...
def epoch_data (MEP_data, upper, lower, timepoint, alias):
    ...
    c = 0          # set count for stim intens list iteration
    if alias in ['011', '014', '015'] and timepoint == 'pre1':
        while len(MEP_data[timepoint]['pre1_180']['TMS_pulse']) > len(MEP_data[timepoint]['pre1_180']['stim_intens']):
                MEP_data[timepoint]['pre1_180']['TMS_pulse'].pop()
        while len(MEP_data[timepoint]['pre1_0']['TMS_pulse']) > len(MEP_data[timepoint]['pre1_0']['stim_intens']):
                MEP_data[timepoint]['pre1_0']['TMS_pulse'].pop()        
        for artefact in MEP_data[timepoint]['pre1_0']['TMS_pulse']: # iterate over marker
            epoch[str(i)]['intensity'] = MEP_data[timepoint]['pre1_0']['stim_intens'][c]
...
for alias in subject_ids: #iterate over subjects and load files
    if len(alias) == 3:
        MEP_data = pd.read_json(direct + alias + '.json').to_dict()
