...
preceeded by script 3 'stroke_data_epoch' and followed by 5 'stroke_data_sort'
...
def plot_trials(data, subject_data, delay, alias, timepoint):
    ...
    for epoch_nr in data.keys():
        ...
        subject_data[epoch_nr]['latency'] = latency
        if trial['intensity']:
...
for timepoint in timepoints:
    ...
    for alias in subjects:
        ...
        if os.path.exists(direct + 'sorted/' + timepoint): # check if folder already exists
            df.to_json (direct + 'sorted/' +  timepoint + '/' + alias )
        else:
            os.makedirs(direct + 'sorted/' +  timepoint) # otherwise create folder
            df.to_json (direct + 'sorted/' +  timepoint + '/' + alias )
