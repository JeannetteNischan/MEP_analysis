...
if os.path.exists(direct + '/timepoints_visual.json'): 
        with open(direct +  '/timepoints_visual.json') as json_file:
            timepoints = json.load(json_file)
else:            
    timepoints = os.listdir(direct)
    timepoints.remove('labelled')
...
for timepoint in timepoints:
    #check if subject list file exists, otherwise create list
    if os.path.exists(target_direct + timepoint + '/subjects_visual.json'): 
        with open(target_direct + timepoint + '/subjects_visual.json') as json_file:
            subjects = json.load(json_file)
    else:
        subjects = os.listdir(direct + timepoint)
        subjects.remove('subjects.json')
    ...
    for alias in subjects:
        ...
        with open(target_direct + timepoint + '/subjects_visual.json', 'w') as file:
            json.dump(subjects_list, file)

    timepoints_list  = timepoints.copy()
    timepoints_list.remove(timepoint)
    with open(direct + '/timepoints_visual.json', 'w') as file:
            json.dump(timepoints_list, file)
    if len(subjects_list) == 0:
        os.remove(target_direct + timepoint + '/subjects_visual.json')    
    if len(timepoints_list) == 0:
        os.remove(direct + '/timepoints_visual.json')    